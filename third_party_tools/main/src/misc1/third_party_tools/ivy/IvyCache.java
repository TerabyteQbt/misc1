...
public class IvyCache {
    ...
    public IvyResult queryIvy(final IvyModuleAndVersion mv) {
        ...
        QbtUtils.semiAtomicDirCache(moduleDir, "", new Function<Path, ObjectUtils.Null>() {
            ...
            public ObjectUtils.Null apply(Path moduleTempDir) {
                try {
                    Pair<ImmutableList<IvyModuleAndVersion>, ImmutableMap<IvyModuleAndVersion, ImmutableList<Pair<String, String>>>> depsAndLicenses = ivyResolveAndRetrieve(mv, null, ImmutableList.of("master"));
                    ImmutableList<IvyModuleAndVersion> deps = depsAndLicenses.getLeft();
                    ...
                    ImmutableList<Pair<String, String>> licenses = depsAndLicenses.getRight().get(mv);
                    boolean licenseFound = false;
                    // TODO: if license was overridden in config file, write
                    // that instead and return
                    for(Pair<String, String> license : licenses) {
                        // TODO: try to get license from URL (pair.right)
                        licenseFound = true;
                    }
                    if(!licenseFound) {
                        // TODO: write "unknown" license file
                    }
     ...
     *
     */
    private static Pair<ImmutableList<IvyModuleAndVersion>, ImmutableMap<IvyModuleAndVersion, ImmutableList<Pair<String, String>>>> ivyResolveAndRetrieve(IvyModuleAndVersion module, Path destinationPath, ImmutableList<String> configs) throws ParseException, IOException {
        ...
        try(QbtTempDir cacheDir = new QbtTempDir()) {
            ...
            ImmutableMap.Builder<IvyModuleAndVersion, ImmutableList<Pair<String, String>>> licenseMap = ImmutableMap.builder();
            ModuleRevisionId ourMrid = m.getModuleRevisionId();
            licenseMap.put(new IvyModuleAndVersion(ourMrid.getOrganisation(), ourMrid.getName(), ourMrid.getRevision()), getLicensesAsStrings(ds.getLicenses()));
            for(DependencyDescriptor depD : ds.getDependencies()) {
                ModuleRevisionId mrid = depD.getDependencyRevisionId();
                IvyModuleAndVersion imav = new IvyModuleAndVersion(mrid.getOrganisation(), mrid.getName(), mrid.getRevision());
                dependencyModuleStrings.add(imav);
            }
            // now retrieve
            ...
            return Pair.of(dependencyModuleStrings.build(), licenseMap.build());
        }
    }

    private static ImmutableList<Pair<String, String>> getLicensesAsStrings(License[] licenses) {
        return ImmutableList.copyOf(Iterables.transform(Arrays.asList(licenses), new Function<License, Pair<String, String>>() {
            @Override
            public Pair<String, String> apply(License input) {
                return Pair.of(input.getName(), input.getUrl());
            }
        }));
    }
}
